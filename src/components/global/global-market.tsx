      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold">HireNova Global</h1>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { val: total, label: 'Offres', icon: Briefcase },
            { val: '40+', label: 'Pays', icon: MapPin },
            { val: '6', label: 'Régions', icon: Globe }
          ].map((s, i) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          ...
          <div className="relative">
            ...
            <Input placeholder="Pays..." value={country} onChange={e => { setCountry(e.target.value); setPage(1) }} className="pl-9" />
          </div>
        ...
        {!loading && jobs.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <p className="text-lg font-medium text-muted-foreground">Aucune offre internationale trouvée</p>
            <p className="text-sm text-muted-foreground/70 mt-1">Explorez les offres dans différentes régions</p>
          </motion.div>
        )}
        {!loading && jobs.length > 0 && (
          <>
            <p className="text-sm text-muted-foreground mb-4">{total} offre{total > 1 ? 's' : ''} internationale{total > 1 ? 's' : ''}</p>
                  ...
                  <Card className="hover:shadow-lg transition-all cursor-pointer h-full border hover:border-teal-200" onClick={() => setStep('globalJobDetail', { jobId: job.id })}>
                    <CardContent className="p-5 flex flex-col h-full">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <Badge variant="secondary" className="text-xs ml-2 flex-shrink-0">{job.region}</Badge>
                      </div>
                      <div className="space-y-1.5 text-xs text-muted-foreground flex-1">
                        <p className="flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> {job.location}, {countryFlags[job.country] || '🌍'} {job.country}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {job.visaSponsorship && <Badge className="text-xs bg-emerald-100 text-emerald-700 hover:bg-emerald-100"><Shield className="w-2.5 h-2.5 mr-0.5" /> Visa</Badge>}
                          {job.relocationPackage && <Badge className="text-xs bg-blue-100 text-blue-700 hover:bg-blue-100"><Plane className="w-2.5 h-2.5 mr-0.5" /> Relocation</Badge>}
                          {job.isRemote && <Badge className="text-xs bg-purple-100 text-purple-700 hover:bg-purple-100"><Wifi className="w-2.5 h-2.5 mr-0.5" /> Remote</Badge>}
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-3 pt-3 border-t">
                        ...
                        <Button size="sm" className="bg-teal-600 hover:bg-teal-700 text-xs cursor-pointer">Voir l&apos;offre</Button>
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
